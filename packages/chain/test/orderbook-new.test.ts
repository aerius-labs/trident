import {OrderBook, Order, MAX_MATCHES} from '../src/runtime/orderbook-new/orderbook';
...
describe('OrderBook', () => {
    ...
    describe('Order Matching', () => {
        test('should match orders with exact price match and fill with empty orders', () => {
            orderBook.addOrder(new Order('1', 50000, 1, 'BTC', 'USD', true, Date.now()));
            ...
            for (let i = 1; i < MAX_MATCHES; i++) {
                expect(matches[i][0].id).toBe('');
                expect(matches[i][1].id).toBe('');
            }
        });

        test('should not match orders with different prices and return all empty orders', () => {
            orderBook.addOrder(new Order('1', 50000, 1, 'BTC', 'USD', true, Date.now()));
        ...
        test('should handle partial matches correctly and fill remaining with empty orders', () => {
            orderBook.addOrder(new Order('1', 50000, 1, 'BTC', 'USD', true, Date.now()));
            orderBook.addOrder(new Order('2', 1/50000, 25000, 'USD', 'BTC', false, Date.now()));
